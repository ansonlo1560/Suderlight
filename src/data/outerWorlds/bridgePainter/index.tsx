...
export const buildings: Building[] = [
  ...
  },
  {
    ...
    baseColor: '#b5651d',
    windows: [
      { side: 'left', x: 0.15, y: 0.15, w: 0.18, h: 0.28 },
      { side: 'left', x: 0.42, y: 0.15, w: 0.18, h: 0.28 },
      { side: 'left', x: 0.68, y: 0.15, w: 0.18, h: 0.28 },
      { side: 'left', x: 0.15, y: 0.55, w: 0.18, h: 0.28 },
      { side: 'left', x: 0.42, y: 0.55, w: 0.18, h: 0.28 },
      { side: 'left', x: 0.68, y: 0.55, w: 0.18, h: 0.28 },
      { side: 'right', x: 0.2, y: 0.15, w: 0.2, h: 0.28 },
      { side: 'right', x: 0.6, y: 0.15, w: 0.2, h: 0.28 },
      { side: 'right', x: 0.2, y: 0.55, w: 0.2, h: 0.28 },
      { side: 'right', x: 0.6, y: 0.55, w: 0.2, h: 0.28 },
    ],
    // decorations: ({ points, isRepaired }) => (
    //   <div style={{
    //     position: 'absolute',
    //     left: points.s2.left + 85,
    //     top: points.t2.top - 95,
    //     transform: 'skewY(-26.5deg)',
    //     background: isRepaired ? 'rgba(181,101,29,0.92)' : 'rgba(30,30,34,0.9)',
    ...
    //     fontWeight: 'bold',
    //     letterSpacing: '2px',
    //     boxShadow: isRepaired ? '0 0 20px rgba(255,213,79,0.5)' : 'none',
    //     pointerEvents: 'none',
    //     userSelect: 'none',
    //     zIndex: 10,
    //     textAlign: 'center',
  ...
  if (entityId === 'rena') {
    ...
    if (ctx.npcEnding === 'failed') {
      return {
        title: '面具的餘音',
        content: [
          '對話結束時，蕾娜對你露出了一個完美的笑容——那笑容精準、明亮、無懈可擊，但你看到她的眼神深處，最後一絲光熄滅了。',
          '',
          '從那以後，她依然每晚登台，但站在聚光燈下的與其說是人，不如說是一座笑臉的雕像。',
          '笑話一個接一個，觀眾的笑聲一次比一次響，但仔細聽，她的語氣裡已經沒有任何起伏，像一台設定好頻率的播放器。',
          '',
          '後台的那面鏡子不知何時碎了。',
          '裂縫正好穿過那張口紅畫出來的笑臉，把它切成兩半。',
          '沒有人來修——反正她在鏡子裡也只看得見同一張臉。',
          '',
          '那本沾有淚痕的笑話集被塞在櫃子最深處，翻到最後一頁，紙張被指甲反覆刮爛，留下兩個字：',
          '「好累。」',
          '',
          '她再也沒去掃過父親的墓。久而久之，她連「為什麼要掃墓」都忘了。',
          '',
          '合約到期那天，她沒有續約。經紀人追問原因，她只是笑了笑，說：「沒意義了。」',
          '沒有人知道她去了哪裡，也沒有人找她。',
          '海報上她的名字被疊上了新的頭像，很快便褪了色。',
          '',
          '面具徹底長進了皮膚裡，她變成了再無表情的空殼——但偶爾有老觀眾提起她時，仍會說：',
          '「蕾娜那時候，笑得特別開心呢。」',
        ].join('\n'),
        actions: [{ label: '查看餘波匯報', tone: 'primary', onClick: ctx.onOpenReport }],
      };
