...
export default function EmotionDictionaryPage({ onBack }: EmotionDictionaryPageProps) {
  ...
  const filteredEntries = useMemo(() => {
    return entries.filter(entry => {
      // 根據 relatedClues[0] 或 unlockCondition 判斷 NPC
  ...
  return (
    <GuiFrame tone="paper">
      ...
      <div style={{ position: 'relative', zIndex: 2, height: '100%', display: 'grid', gridTemplateColumns: '320px minmax(520px, 820px)', gap: 24, alignItems: 'center', justifyContent: 'center', padding: 32 }}>
        <GlassPanel title="情緒詞典" subtitle="Emotion Dictionary" variant="paper" contentStyle={{ display: 'flex', flexDirection: 'column', height: '100%', maxHeight: '70vh', padding: '16px 20px' }}>
          ...
          <div className="dictionary-scroll" style={{ flex: 1, overflowY: 'auto', display: 'grid', gap: 10, paddingRight: 6 }}>
          ...
          <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid rgba(95,61,32,0.1)' }}>
        ...
        <GlassPanel title={selected?.name ?? '未選擇詞條'} subtitle="Welfare Card" variant="paper" style={{ minHeight: 560 }} contentStyle={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, alignItems: 'stretch' }}>
          <div style={{ borderRadius: 16, padding: 24, background: 'rgba(35,29,24,0.86)', color: '#f4efe7', boxShadow: 'inset 0 0 60px rgba(0,0,0,0.35)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ borderRadius: 16, padding: 24, background: 'linear-gradient(180deg, rgba(255,246,216,0.92), rgba(221,237,213,0.9))', color: '#293226', border: '1px solid rgba(80,108,76,0.2)', display: 'flex', flexDirection: 'column' }}>
            <div style={{ color: '#678c5a', letterSpacing: 3, fontSize: 12 }}>右頁：現實支持提醒</div>
