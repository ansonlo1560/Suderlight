...
import brushImage from '../../images/painter/map-brush.png';
import newspaperImage from '../../images/painter/map-newsPaper.png';
import sketchbookImage from '../../images/painter/map-sketchPaper.png';
...
import painterImage from '../../images/painter/painter.png';
import painterUnlockedImage from '../../images/painter/painterSuccess.png';
    ...
    <div style={{ width: '100vw', height: '100vh', position: 'relative', overflow: 'hidden', cursor: isDragging ? 'grabbing' : 'grab', background: '#080a0d', filter: traumaFilter }} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
      ...
      <div style={{ position: 'absolute', transform: `translate(${mapPos.x}px, ${mapPos.y}px)`, width: world.mapWidth, height: world.mapHeight }}>
        ...
        {visibleBuildings.map(b => (
          <IsometricBuilding key={b.id} building={b} isRepaired={b.id === 'tavern' ? true : b.id === 'pavilion' ? save.npcs['aoi']?.ending === 'success' : b.id === 'theater' ? save.npcs['rena']?.ending === 'success' : isRepaired} mapWidth={world.mapWidth} mapHeight={world.mapHeight} />
        ))}
        {entities.map(entity => {
          ...
          const bw = isTC ? 82 : (isImg ? 96 : (isPill ? 94 : (isAoi ? 100 : (isRena ? 72 : (entity.type === 'npc' ? 64 : 48)))));
          const bh = isTC ? 82 : (isImg ? 96 : (isPill ? 36 : (isAoi ? 140 : (isRena ? 100 : (entity.type === 'npc' ? 84 : 48)))));
          return (
            <button key={entity.id} onClick={e => handleEntityClick(e, entity)} style={{ position: 'absolute', left: s.left, top: s.top, transform: 'translate(-50%, -100%)', width: bw, height: bh, border: isTC ? '2px dashed #5a5a6e' : ((isPtr || isAoi || isRena || isImg) ? 'none' : `2px solid ${entity.color}`), borderRadius: (isPtr || isAoi || isRena) ? '0' : isImg ? 0 : isTC ? '8px 14px 10px 4px' : isPill ? '999px' : entity.type === 'npc' ? '36px 36px 18px 18px' : '50%', padding: (isPtr || isAoi || isRena || isImg) ? '0' : isTC ? '4px' : isPill ? '0 8px' : '0', background: isTC ? 'rgba(20,22,30,0.94)' : (isPtr || isAoi || isRena || isImg) ? 'transparent' : entity.type === 'npc' ? 'rgba(255,170,51,0.12)' : 'rgba(255,255,255,0.08)', color: isTC ? '#8a8a9c' : entity.color, cursor: 'pointer', zIndex: Math.round(s.top) + ((isGDoor || isTDoor || entity.id === 'rena') ? 500 : 0), boxShadow: isTC ? (isNear ? '0 0 28px rgba(120,120,140,0.35)' : '0 0 10px rgba(120,120,140,0.15)') : isPtr ? 'none' : isAoi ? 'none' : isRena ? 'none' : isImg ? 'none' : (isNear ? `0 0 36px ${entity.color}` : `0 0 18px ${entity.color}55`), fontWeight: 'bold', userSelect: 'none', transition: 'box-shadow 0.18s, transform 0.18s', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              ...
              : isAoi ? <img src={save.npcs['aoi']?.ending === 'failed' ? aoiGoneImage : aoiImage} alt={entity.label} style={{ width: '100%', height: '100%', objectFit: 'contain', objectPosition: 'center bottom', border: 'none', borderRadius: 0, filter: save.npcs['aoi']?.ending === 'failed' ? 'none' : save.npcs['aoi']?.ending === 'success' ? (isNear ? 'drop-shadow(0 0 20px rgba(255,170,51,0.45))' : 'none') : isNear ? 'grayscale(1) drop-shadow(0 0 20px rgba(255,170,51,0.45))' : 'grayscale(1)' }} />
              : isRena ? <img src={save.npcs['rena']?.ending === 'failed' ? renaGoneImage : save.npcs['rena']?.ending === 'success' ? renaSuccessImage : renaImage} alt={entity.label} style={{ width: '100%', height: '100%', objectFit: 'contain', objectPosition: 'center bottom', border: 'none', borderRadius: 0, filter: save.npcs['rena']?.ending === 'failed' ? 'none' : save.npcs['rena']?.ending === 'success' ? (isNear ? 'drop-shadow(0 0 20px rgba(255,170,51,0.45))' : 'none') : isNear ? 'grayscale(1) drop-shadow(0 0 20px rgba(255,170,51,0.45))' : 'grayscale(1)' }} />
              : isImg && cImg ? <img src={cImg} alt={entity.label} style={{ width: '100%', height: '100%', objectFit: 'contain', borderRadius: 6, border: 'none', filter: isNear ? `drop-shadow(0 0 20px ${entity.color})` : 'none', transition: 'filter 0.18s' }} />
