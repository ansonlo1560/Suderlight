...
export default function NarrativeDebugOverlay({ currentScreen }: Props) {
  ...
  return (
    <div style={PANEL}>
      ...
      {/* ---- screen indicator ---- */}
      <div style={{ color: '#7ec8ff', fontSize: 14, fontWeight: 700, marginBottom: 10, letterSpacing: 0.5, display: 'flex', alignItems: 'center', flexWrap: 'wrap' }}>
        当前地图：{locations[save.currentLocation as LocationId].name} ({save.currentLocation})
      </div>

      {/* ---- hotkeys & npc switcher ---- */}
      <div style={{ ...SECTION, padding: '10px 12px', marginBottom: 10 }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px 16px', fontSize: 13, color: '#889', marginBottom: 10 }}>
          <span><b style={{color:'#7ec8ff'}}>F7</b> 强制解锁</span>
          <span><b style={{color:'#7ec8ff'}}>F8</b> 切换面板</span>
          <span><b style={{color:'#7ec8ff'}}>F9</b> 进入内心</span>
          <span><b style={{color:'#7ec8ff'}}>S+F9</b> 选章节</span>
          <span><b style={{color:'#ff9800'}}>F10</b> Demo</span>
        </div>
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 10, display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
          <span style={{ color: '#7ec8ff', fontSize: 12, fontWeight: 700, letterSpacing: 0.5 }}>角色切换：</span>
          {(Object.keys(save.npcs) as NpcId[]).map((id) => (
            <button
              ...
              style={{
                ...
                fontFamily: 'inherit',
                fontSize: 12,
                fontWeight: activeNpcId === id ? 700 : 500,
